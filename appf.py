...
stopwords_file_path = 'stopwords.txt'  # Replace with the actual path

# Read stop words from the file
with open(stopwords_file_path, encoding="utf8") as file:
...
def sentence_embedding(sentence):
    """
    Generate the sentence embedding by averaging the word embeddings.
    """
    words = word_tokenize(sentence)
    word_embeddings = [get_word_embedding(word.lower()) for word in words if word.lower() not in ENGLISH_STOP_WORDS]
    if word_embeddings:
        return np.mean(word_embeddings, axis=0)
    else:
        return np.zeros(word2vec_model.vector_size)

def _score_sentences_with_embedding(sentences):
    """
    Score sentences based on cosine similarity using Word2Vec embeddings.
    """
    sentence_embeddings = {sent: sentence_embedding(sent) for sent in sentences}
    similarity_matrix = np.zeros((len(sentences), len(sentences)))

    for i in range(len(sentences)):
        for j in range(len(sentences)):
            if i != j:
                similarity_matrix[i][j] = np.dot(sentence_embeddings[sentences[i]], sentence_embeddings[sentences[j]])
                
    # Normalize the similarity scores
    similarity_matrix /= np.linalg.norm(similarity_matrix, axis=1, keepdims=True)

    sentence_scores = dict(zip(sentences, np.sum(similarity_matrix, axis=1)))

    return sentence_scores

# Now, modify your _score_sentences function to use the embedding-based scoring
def _score_sentences(tf_idf_matrix, tf_gss_matrix, sentences):
    """
    Score a sentence by combining TF-IDF, TF_GSS, and Word Embedding scores.
    """
    alpha = 0.5
    beta = 0.3
    gamma = 0.2

    # Use the Word Embedding scores
    sentence_scores_embedding = _score_sentences_with_embedding(sentences)

    # Use the previous TF-IDF and TF_GSS scores
    sentence_scores_tfidf = _score_sentences(tf_idf_matrix, tf_gss_matrix)

    # Combine the scores
    for sent in sentences:
        if sent in sentence_scores_tfidf and sent in sentence_scores_embedding:
            sentence_scores_tfidf[sent] = (alpha * sentence_scores_tfidf[sent]) + (beta * sentence_scores_embedding[sent]) + (gamma * sentence_scores_tfidf[sent])

    return sentence_scores_tfidf
...
# Function to create documents per words
